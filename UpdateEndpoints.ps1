...
$SB = {
    #Enter-PSSession -ComputerName oecu-maxxar -Credential oecu\mhampton
    ...
    if ((Test-Path C:\Windows\SoftwareDistribution)) {
        ...
        $Qty = $updates.count
        Get-HotFix -erroraction 'silentlycontinue' | Sort-Object InstalledOn -Descending -erroraction 'silentlycontinue' | convertto-csv | out-file c:\WindowsUpdates\Old-hotfixes.csv
        ...
        if ($Qty -eq 0){
            ...
            if ($errorLogs.count -gt 0) {
                ...
                Remove-Item C:\WindowsUpdates\* -Include "*.msu", "*.exe";
...
#$results = Invoke-Command -ComputerName $serverInvokeList -ScriptBlock $SB -Credential OECU\mhampton
